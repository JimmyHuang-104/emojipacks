...
function Slack(opts, debug) {
  ...
  this.import = function*() {
    ...
    yield this.batchProcessing(this.opts.emojis);
    return 'Success';
  };
   ...
   * Batch processing
   */
  this.batchProcessing = function*(emojis) {
    console.log('batchProcessing start');
    for (const emoji of emojis) {
      if (emoji.src) {
        yield this.upload(emoji.name, emoji.src);
      }
      const uploadAlias = this.alias;
      if (emoji.aliases) {
        emoji.aliases.forEach(function*(alias) {
          yield uploadAlias(emoji.name, alias);
        });
      }
    }
  };
